...
namespace Dalssoft.DiagramNet
{
    public class Designer : System.Windows.Forms.UserControl
    {
        ...
        private Cuadro_texto_nodo connStart;
        private Cuadro_texto_nodo connEnd;
                ...
                case DesignerAction.Select:
                    if (e.Button == MouseButtons.Left)
                    {
                        ...
                        if (selectedElement != null)
                        {
                            ...
                            // Element selected
                            if (selectedElement is Cuadro_texto_nodo)
                            {
                                StartAddLink((Cuadro_texto_nodo)selectedElement, mousePoint);
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.None)
            {
                ...
                if (document.Action == DesignerAction.Connect)
                {
                    ...
                    {
                        if (mousePointerElementTMP is Cuadro_texto_nodo)
                        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (isAddLink)
                {
                    ...
                    if ((selectedElement is Cuadro_texto_nodo)
                        && (document.CanAddLink(connStart, (Cuadro_texto_nodo)selectedElement)))
                        linkLine.Connector2 = (Cuadro_texto_nodo)selectedElement;
                    else
		...
		#region Link
		private void StartAddLink(Cuadro_texto_nodo connStart, Point mousePoint)
		{
			if (document.Action == DesignerAction.Connect)
			{
				this.connStart = connStart;
				this.connEnd = new Cuadro_texto_nodo(connStart.ParentElement);
		...
		private void EndAddElement(Rectangle selectionRectangle)
		{
			...
			switch (document.ElementType)
			{
				...
				case ElementType.RectangleNode:
					el = new Rectangulo_con_nodo(selectionRectangle);
					break;
				case ElementType.Elipse:
					el = new Circulo(selectionRectangle);
					break;
				case ElementType.ElipseNode:
					el = new Circulo_con_nodo(selectionRectangle);
					break;
				case ElementType.CommentBox:
					el = new Cuadro_texto(selectionRectangle);
					break;
