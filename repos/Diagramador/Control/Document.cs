...
namespace Dalssoft.DiagramNet
{
	...
	public class Document: IDeserializationCallback 
	{
		...
		internal bool CanAddLink(Cuadro_texto_nodo connStart, Cuadro_texto_nodo connEnd)
		{
			return ((connStart != connEnd) && (connStart.ParentElement != connEnd.ParentElement));
		}

		public BaseLinkElement AddLink(Cuadro_texto_nodo connStart, Cuadro_texto_nodo connEnd)
		{
		...
		{
			if ((el != null) && !(el is Cuadro_texto_nodo))
			{
				//Delete link
				...
				if (el is NodeElement)
				{
					...
					foreach (Cuadro_texto_nodo elconn in conn.Connectors)
					{
						BaseLinkElement lnk;
		...
		public void SelectElements(Rectangle selectionRectangle)
		{
			...
			foreach(BaseElement element in elements)
			{
				if (element is IControllable)
				{
					...
					{
						if (!(element is Cuadro_texto_nodo))
			...
			if ((elements != null) && (elements.Count > 0))
			{
				...
				for(int i = elements.Count - 1; i >=0 ; i--)
				{
					...
					if (el is NodeElement)
					{
						...
						foreach(Cuadro_texto_nodo cel in nel.Connectors)
						{
